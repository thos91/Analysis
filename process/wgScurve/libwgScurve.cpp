...
void MakeScurveXML(string& outputXMLDir, Topology& topol) {
  wgEditXML Edit;
  for(unsigned idif_id = 1; idif_id <= topol.n_difs; idif_id++) {
    for(unsigned ichip_id = 1; ichip_id < topol.dif_map[idif].size(); ichip_id++) {
      MakeDir(outputXMLDir + "/dif" + to_string(idif_id) + "/chip" + to_string(ichip_id));
      for(unsigned ichan_id = 1; ichan_id < topol.dif_map[idif][ichip]; ichan_id++) {
