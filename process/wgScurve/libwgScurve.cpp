...
int wgScurve(const char* x_input_dir,
  ...
  try{
    ...
    d1vector mean1PE(n_inputDAC), mean2PE(n_inputDAC), sigma1PE(n_inputDAC), sigma2PE(n_inputDAC);
    ...
    std::vector<TH1D*> Pe1Hist(n_inputDAC);
    std::vector<TH1D*> Pe2Hist(n_inputDAC);
    std::vector<TH1D*> ChiHist(n_inputDAC);
    std::vector<TH1D*> ChiOverNdfHist(n_inputDAC);
    for(unsigned i_iDAC = 0; i_iDAC < n_inputDAC; ++i_iDAC){
    ...
    for(unsigned i_iDAC = 0; i_iDAC < n_inputDAC; ++i_iDAC){
      ...
      TString name(output_img_dir + "/EvaluationOfFit/iDAC_" + std::to_string(inputDAC[i_iDAC]) +  ".png");
      PECanvas->Print(name);
