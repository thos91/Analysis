...
int wgDecoder(const char * x_input_raw_file,
  ...
  bitset<M> raw_data; 
  vector<bitset<M>> raw_data_stack;
  // The last four raw_data values are kept in the last_four_raw_data array
  bitset<M> last_four_raw_data[4];
  ...
  while (ifs.read((char*) &raw_data, M / 8) && ievent < maxEvt) {
    raw_data_stack.push_back(raw_data);
    ...
    // *********   CHIP TRAILER  ********* //
    if (last_four_raw_data[1] == xFFFE && raw_data == x2020) {
      endOfChipTag = true;
    }

    // *********   SPILL TRAILER  ********* //
    if (last_four_raw_data[2] == x2020 && last_four_raw_data[1] == xFFFF) {
      endOfSpillTag = true;
    }
    ...
    if (endOfSpillTag && last_four_raw_data[2] == xFFFB) {
      ...
      // Spill number
      SPILL_NUMBER = last_four_raw_data[1].to_ulong();
    ...
    if( last_four_raw_data[1] == xFFFC && endOfSpillTag ){
      ...
      if( endOfSpillTag ) {
        ...
        bitset<2*M> spill_count_msb = last_four_raw_data[0].to_ulong();
        spill_count_msb <<= M;
        // Spill count least significant byte
        bitset<2*M> spill_count_lsb = raw_data.to_ulong();
    ...
    if (last_four_raw_data[2] == x2020 && last_four_raw_data[1] == xFFFD){
    ...
    if (First_Event && last_four_raw_data[3] == x2020 && last_four_raw_data[2] == xFFFF) {

      // Read the number of chips from the spill trailer
      uint16_t nbchip = ( raw_data & x00FF ).to_ulong();
      if( n_found_chips != nbchip ) {
        // maybe one word was skipped
        nbchip = ( last_four_raw_data[0] & x00FF ).to_ulong();
        if( n_found_chips != nbchip ){
      ...
      if (n_found_chips > 0) {
        ...
        vector<bitset<M>>& eventData  = raw_data_stack;
        ...
        for (size_t i = 0; i < eventData.size(); i++) {
          ...
          if (last == xFFFE) { 
            ...
            if (isValidChip) {
              // Check the raw_dataSize
              raw_dataSize = i - chipStartIndex - CHIPENDTAG - offset;
              nColumns    = (raw_dataSize - CHIPIDSIZE) / (1 + NCHANNELS * 2);
              if ( (raw_dataSize - CHIPIDSIZE) % (1 + NCHANNELS * 2) != 0) {
    ...
    if (last_four_raw_data[1] == x5053 && last_four_raw_data[0] == x4C49 &&  raw_data == x2020) { 
      if (endOfChipTag) {
    ...
    last_four_raw_data[3] = last_four_raw_data[2];
    last_four_raw_data[2] = last_four_raw_data[1];
...
unsigned GetNumChipID(string & input_raw_file) {
  ...
  bitset<M> raw_data;
  unsigned nRawData = 0;
  // The last four raw_data values are kept in the lastFour array
  bitset<M> last_four_raw_data[4];
  last_four_raw_data[0] = x2020;
  ...
  // Read M / 8 bytes of raw data and put it into raw_data
  while ( ifs.read((char * ) &raw_data, M / 8) ) {

    // CHIP HEADER :
    // Reset the counter
    if (last_four_raw_data[2] == x4843 && last_four_raw_data[1] == x5049 && last_four_raw_data[0] == x2020 ) {
      nRawData = 0;
    }
    ...
    // CHIP TRAILER :
    // Count the columns
    if (last_four_raw_data[3] == xFFFE && last_four_raw_data[1] == x2020 && last_four_raw_data[0] == x2020 ) {
