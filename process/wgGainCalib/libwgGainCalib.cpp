...
int wgGainCalib(const char * x_input_run_dir,
        ...
        for (auto const& chip: dif.second) {
          ...
          for (unsigned ichan = 0; ichan < chip.second; ++ichan) {
            ...
            for (unsigned icol = 0; icol < MEMDEPTH; ++icol) {
              ...
              charge_hit[idif][ichip][ichan][icol][i_idac][ipe] = Edit.SUMMARY_GetChFitValue("charge_hit_" + std::to_string(icol), ichan);
              sigma_hit [idif][ichip][ichan][icol][i_idac][ipe] = Edit.SUMMARY_GetChFitValue("sigma_hit_"  + std::to_string(icol), ichan);
            }
          }
          Edit.Close();
        }
      ...
      for (unsigned ichan = 0; ichan < chip.second; ++ichan) {
        ...
        for (unsigned icol = 0; icol < MEMDEPTH; ++icol) {
          ...
          for (auto const& i_idac : v_idac) {
            gain      [idif][ichip][ichan][icol][i_idac] = charge_hit[idif][ichip][ichan][icol][i_idac][TWO_PE] - charge_hit[idif][ichip][ichan][icol][i_idac][ONE_PE];
            sigma_gain[idif][ichip][ichan][icol][i_idac] = std::sqrt(std::pow(sigma_hit[idif][ichip][ichan][icol][i_idac][TWO_PE], 2) +
        ...
        multi_graph->Draw("ap");
        TString image;
        image.Form("%s/dif%d/chip%d/inputDAC_vs_gain_chan%d.png", output_img_dir.c_str(), idif, ichip, ichan);
        canvas->Print(image);
  ...
  for (auto const& dif: topol.dif_map) {
    ...
    for (auto const& chip: dif.second) {
      ...
      for (unsigned ichan = 0; ichan < chip.second; ++ichan) {
        for (unsigned icol = 0; icol < MEMDEPTH; ++icol) {
          Edit.GainCalib_SetValue(std::string("slope_gain"),     idif, ichip, ichan, icol, slope    [idif][ichip][ichan][icol], NO_CREATE_NEW_MODE);
          Edit.GainCalib_SetValue(std::string("intercept_gain"), idif, ichip, ichan, icol, intercept[idif][ichip][ichan][icol], NO_CREATE_NEW_MODE);
        }
      }
    }
  }
