...
void wgFit::NoiseRate(unsigned ichip, unsigned ichan, double (&x)[2], bool print_flag) {
  TH1I * bcid_hit = wgFit::histos_.Get_bcid_hit(ichip, ichan);
  if (bcid_hit == NULL) {
    x[0] = x[1] = std::nan("");
...
void wgFit::ChargeHit(TH1I * charge_hit, double (&x)[3], wgFit::GainSelect gs) {
  // Find the right-most bin that is non-zero.
  ...
  unsigned begin, end;
  if (gs == wgFit::GainSelect::HIGH_GAIN) {
    begin = WG_BEGIN_CHARGE_HIT_HG;
    end = WG_END_CHARGE_HIT_HG;
  } else if (gs == wgFit::GainSelect::LOW_GAIN) {
...
//**********************************************************************
void wgFit::ChargeHitHG(unsigned ichip, unsigned ichan, unsigned icol,
  ...
  wgFit::ChargeHit(charge_hit_HG, x, wgFit::GainSelect::HIGH_GAIN);
  ...
  if( print_flag && (!m_output_img_dir.empty()) ) {
    ...
    wgFit::histos_.Print_charge_hit_HG(image, charge_hit_HG);
  }
  delete charge_hit_HG;
}

//**********************************************************************
void wgFit::ChargeHitLG(unsigned ichip, unsigned ichan, unsigned icol,
                       double (&x)[3], bool print_flag) {
  TH1I * charge_hit_LG = wgFit::histos_.Get_charge_hit_LG(ichip, ichan, icol);
  ...
  wgFit::ChargeHit(charge_hit_LG, x, wgFit::GainSelect::LOW_GAIN);
...
//**********************************************************************
void wgFit::ChargeNohit(unsigned ichip, unsigned ichan, unsigned icol,
                         double (&x)[3], bool print_flag) {
  TH1I * charge_nohit = wgFit::histos_.Get_charge_nohit(ichip, ichan, icol);
  ...
  wgFit::ChargeHit(charge_nohit, x, wgFit::GainSelect::PEDESTAL);
